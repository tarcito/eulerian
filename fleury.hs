...
depth_First_Search ((Set (x:xs)), (Set ys)) = d_F_S x ys

d_F_S v ys = v: d_F_S filter  ys



-- To Do ...
eh_Conexo ::  (Eq a) => (Set a, Set (a, a)) -> Bool
eh_Conexo ((Set xs), (Set ys)) = [(x, y)| x <- xs, y <- ys] ++ 
...
eulerian_Path grafo |((eh_Conexo grafo) && (eh_Digrafo grafo) && (eh_Euleriano)) = eulerian_Path' grafo
