...
eh_Semi_Euleriano ((Set xs), (Set ys)) =  ( eh_Conexo ((Set xs),(Set ys)) ) && ( eh_Semi_Euleriano' 0 [] ((Set xs),(Set ys)) )

-- To Do ...

-- função auxiliar que verifica se existe um caminho euleriano, contando os vertices de grau impar e ao mesmo tempo formando a lista com os vertices de grau par, que possivelmente contém um caminho euleriano
eh_Semi_Euleriano' z zs ((Set []), (Set ys)) = (z == 2) && eh_Euleriano2 ((Set zs), (Set ys))
eh_Semi_Euleriano' z zs ((Set (x:xs)),(Set ys)) =  if( ((mod (length (filter (\z -> fst z == x) ys)) 2) == 1) ||  ((mod (length (filter (\z -> snd z == x) ys)) 2) == 1) ) then eh_Semi_Euleriano' (z+1) zs ((Set xs), (Set ys))
              else eh_Semi_Euleriano' z (x:zs) ((Set xs), (Set ys))

eh_Euleriano2 ::  Eq a => (Set a, Set (a, a)) -> Bool
eh_Euleriano2 ((Set []), (Set ys)) = True
eh_Euleriano2 ((Set (x:xs)),(Set ys)) =  ((mod (length (filter (\z -> fst z == x) ys)) 2 == 0) || (mod (length (filter (\z -> snd z == x) ys)) 2 == 0)) && 
              eh_Euleriano2 ((Set xs), (Set ys))
...
g3 = addEdges [(1,2),(2,1),(2,3),(3,2),(3,4),(4,3)] (addVertices [1,2,3,4] graph)
